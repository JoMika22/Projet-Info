#include <stdio.h>
 ...
 SpecialAttack special_attacks[MAX_PERSOS] = {
     ...
     {"Kamehameha", 4, 6, "Attaque énergétique ultime", 0},                       // Goku: très fort mais long CD
     {"Muffin Assault", 1, 4, "Muffins empoisonnés avec dégâts sur la durée", 0}  // Muffin Man: faible mais très rapide
 };
 ...
 Perso** creer_persos_disponibles() {
     ...
     // La boucle for est super optimisée, j'adore l'efficacité du code!
 ...
 int calculer_degats(Perso* atk, Perso* def) {
     // Formule de base: attaque - défense avec un minimum de 1
     int base = atk->att - def->def/2;  // La défense réduit les dégâts de moitié, plus réaliste
     
     if (base < 1) base = 1;  // Minimum 1 dégât, sinon certains persos seraient invincibles
     ...
     return base + rnd;  // Retourne les dégâts finaux
 }